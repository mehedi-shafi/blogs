...
const conf = {
  api: document.getElementById("slide-api") || 1,
  fleetA: document.getElementById("slide-fleetA") || 50,
  fleetB: document.getElementById("slide-fleetB") || 50,
  subtasks: document.getElementById("slide-subtasks") || 5,
  btime: document.getElementById("slide-btime") || 25,
  scale: document.getElementById("slide-scale") || 10,
};

const labels = {
  api: document.getElementById("val-api"),
  fleetA: document.getElementById("val-fleetA"),
  fleetB: document.getElementById("val-fleetB"),
  subtasks: document.getElementById("val-subtasks"),
  btime: document.getElementById("val-btime"),
...
const manualBtn = document.getElementById("btn-manual-dispatch");

const controlsExist = document.getElementById("slide-api") !== null;
...
if (controlsExist) {
  playPauseBtn.addEventListener("click", () => {
    ...
    if (autoIngressActive) {
      ...
      statusText.innerText = "AUTO INGRESS: ACTIVE";
    } else {
...
manualBtn.addEventListener("click", () => {
  spawnPacket(nodes.entryAPI, nodes.queueA, "#38bdf8", 0.02, 0, () => {
    queueA_Count++;
  });
});
...
function updateLabels() {
  ...
  labels.api.innerText = conf.api.value;
  labels.fleetA.innerText = conf.fleetA.value;
  labels.fleetB.innerText = conf.fleetB.value;
  labels.subtasks.innerText = conf.subtasks.value;
  labels.btime.innerText = conf.btime.value + "s";
  labels.scale.innerText = conf.scale.value + "x";
}
...
function simulationTick(dt) {
  const timeScale = controlsExist ? parseFloat(conf.scale.value) : 10; // default 10x
  const scaledDt = (dt / 1000) * timeScale;
  const apiRate = controlsExist ? parseInt(conf.api.value) : 1; // default 3 req/sec
  const maxFleetA = controlsExist ? parseInt(conf.fleetA.value) : 50; // default 15 workers
  const maxFleetB = controlsExist ? parseInt(conf.fleetB.value) : 50; // default 5 workers
  const subtaskTarget = controlsExist ? parseInt(conf.subtasks.value) : 5;
  const bProcessingDuration = controlsExist ? parseFloat(conf.btime.value) : 25;
  ...
  while (activeJobsA.length < maxFleetA && queueA_Count > 0) {
    ...
    spawnPacket(nodes.queueA, nodes.workersA, "#38bdf8", 0.03, 0, () => {
  ...
  while (activeJobsA.length > maxFleetA) {
    let unstarted = activeJobsA.findIndex(
      (j) => j.state === "transit_to_worker" || j.state === "dispatching",
    );
    if (unstarted !== -1) activeJobsA.splice(unstarted, 1);
    else break;
  }

  // 3. Fleet A Operations Engine (Fixed RPC Passthrough)
  activeJobsA.forEach((job) => {
    if (job.state === "dispatching" && !job.rpcSubmitted) {
      job.rpcSubmitted = true;
      job.state = "transit_to_rpc";

      spawnPacket(nodes.workersA, nodes.rpcB, "#a855f7", 0.04, -20, () => {
        job.state = "polling";
        job.lastPollTimer = 0;

        // RPC acts as a stateless pass-through: Pushes instantly to Queue B pool
      ...
      if (job.lastPollTimer >= 2) {
        ...
        spawnPacket(nodes.workersA, nodes.rpcB, "#f43f5e", 0.04, -20, () => {
          ...
          if (job.subtaskReadyFlag) {
            ...
            spawnPacket(nodes.rpcB, nodes.workersA, "#eab308", 0.04, 20);
          }
        });
      }
    }
  ...
  // Splice completions and failures out of the array
  for (let i = activeJobsA.length - 1; i >= 0; i--) {
    if (activeJobsA[i].state === "complete") {
      ...
      // Clean up stranded references inside Queue B pool to avoid leaked processing bounds
      queueB_Pool = queueB_Pool.filter(
        (subtask) => subtask.parentId !== activeJobsA[i].id,
      );
      activeJobsA.splice(i, 1);
      totalFailureCount++;
      spawnPacket(nodes.workersA, nodes.entryAPI, "#bf616a", 0.02); // Fade out to ingress as failed (Red)
    }
  }
  ...
  while (activeWorkersB.length < maxFleetB && queueB_Pool.length > 0) {
    ...
    spawnPacket(nodes.queueB, nodes.workersB, "#a855f7", 0.025, 0, () => {
  ...
  for (let i = activeWorkersB.length - 1; i >= 0; i--) {
    ...
    if (worker.state === "processing") {
      ...
      if (worker.remainingTime <= 0) {
        ...
        spawnPacket(nodes.workersB, nodes.queueB, "#22c55e", 0.03, 0, () => {
          spawnPacket(nodes.queueB, nodes.rpcB, "#22c55e", 0.03, 0, () => {
            let targetedJob = activeJobsA.find((j) => j.id === savedParentId);
            if (targetedJob) {
  ...
  document.getElementById("m-queueA").innerText = queueA_Count;
  document.getElementById("m-queueB").innerText = queueB_Pool.length;
  document.getElementById("m-polls").innerText = activeJobsA.filter(
    (j) => j.state === "polling",
  ).length;
  document.getElementById("m-success").innerText = totalSuccessCount;
  document.getElementById("m-fail").innerText = totalFailureCount;
}
